...
def color_faces(G, V, coords, faces, filename='graph_faces.pdf', paths=None):
    ...
    for f in np.array(faces):
        plt.gca().add_patch(
            plt.Polygon(coords[f], facecolor=(173 / 256, 216 / 256, 240 / 256, 0.4), ec='k', linewidth=0.3))
...
def generate_dataset(n, m, folder, holes=True):
    ...
    prefix_flows_1hop, targets_1hop, last_nodes_1hop, suffixes_1hop, \
        prefix_flows_2hop, targets_2hop, last_nodes_2hop, suffixes_2hop = path_dataset(G_undir, E, edge_to_idx, paths,
                                                                                       max_degree)

    # reversed
    rev_prefix_flows_1hop, rev_targets_1hop, rev_last_nodes_1hop, rev_suffixes_1hop, \
        rev_prefix_flows_2hop, rev_targets_2hop, rev_last_nodes_2hop, rev_suffixes_2hop = path_dataset(G_undir, E,
                                                                                                       edge_to_idx,
                                                                                                       rev_paths,
                                                                                                       max_degree)
    ...
    filenames = (
    'flows_in', 'B1', 'B2', 'targets', 'train_mask', 'test_mask', 'G_undir', 'coords', 'last_nodes', 'target_nodes',
    'rev_flows_in', 'rev_targets', 'rev_last_nodes', 'rev_target_nodes')
